...
def main():
	...
	vqgan_config = OmegaConf.load("..//StableSR-main/configs/autoencoder/autoencoder_kl_64x64x4_resi.yaml")
	vq_model = load_model_from_config(vqgan_config, opt.vqgan_ckpt)  # 直接用训练的模型
		...
		with precision_scope("cuda"):
			with model.ema_scope():
				...
				for item in img_list_ori:
					...
					cur_image = transform(cur_image)
					cur_image = cur_image.clamp(-1, 1)
					init_image = cur_image #torch.unsqueeze(cur_image, 0)
					init_latent_generator, enc_fea_lq = vq_model.encode(init_image) # vqgan编码输出
					init_latent = model.get_first_stage_encoding(init_latent_generator) # 对角高斯分布
					...
					# reverse sample， latent from unet
					...
					# 保存latent
					save_latent = samples.cpu().numpy()

					x_samples = vq_model.decode(samples * 1. / model.scale_factor, enc_fea_lq)  # 解码输出
					if opt.colorfix_type == 'adain':
					...
					#init_latent = init_latent.cpu().numpy()
					#print(save_latent.shape)
					np.save('../train_data/CFW_trainingdata/latents/' + basename + '.npy', save_latent)
